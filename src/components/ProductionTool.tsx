...
export function ProductionTool({
  ...
  return (
    <div className="flex flex-col gap-4 border border-edge p-4">
      <div className="grid gap-4 sm:grid-cols-4">
        <label className="flex flex-col gap-1.5">
          ...
          <input
            ...
            placeholder="octocat"
            className="border-b border-edge pb-1.5 text-[0.8rem] text-fog focus:border-accent"
          />
        </label>
        <label className="flex flex-col gap-1.5">
          ...
          <input
            ...
            className="border-b border-edge pb-1.5 text-[0.8rem] tabular-nums text-fog focus:border-accent"
          />
        </label>
        <label className="flex flex-col gap-1.5">
          ...
          <select
            ...
            className="border-b border-edge bg-void pb-1.5 text-[0.8rem] text-fog focus:border-accent"
          >
            {VARIANTS.map((v) => (
        ...
        <a
          ...
          className={`rounded-[5px] px-4 py-2 text-[0.66rem] tracking-[0.12em] uppercase transition-all duration-150 ${
            ready
              ...
              // Inactive, not illegible: the missing accent fill carries the
              // state, so the label stays readable instead of fading out.
              : "pointer-events-none cursor-not-allowed border border-line text-dim"
          }`}
        >
          build stl at {mm}mm
        </a>
