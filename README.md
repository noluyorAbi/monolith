...
**Multi colour is wired at the geometry, not yet at the slicer.** The 3MF arrives split into one solid per contribution level (Plinth, Quiet, Steady, Busy, Peak), each a named 3MF object, and the print card lists exactly which filament goes to which part. That is enough for Bambu Studio and Orca to show the plate already broken into the parts MONOLITH chose, each assignable to its own slot in the object list. What is still manual: the file does not itself say "peak days go in slot 2" — `basematerials` is inert in every slicer checked, and a hand written Bambu project file segfaults Bambu Studio, so the automatic assignment lives in the slicer, not in the model. The slot choice in the interface reaches the printed card and the preset, never the 3MF. Next: emit a real Bambu and Orca project 3MF, `model_settings.config` with a per object extruder plus the production extension UUIDs, behind a test that loads it through the CLI, and fall back to the plain 3MF when it cannot be produced.
